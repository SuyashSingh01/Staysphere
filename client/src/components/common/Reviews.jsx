        ...
        <div className="space-y-4 grid grid-cols-1 md:grid-cols-2 gap-4">
          {reviews.map((review, index) => (
            <div
              ...
              <p className="text-sm text-gray-700 mt-1">{review?.timestamp}</p>
              </div>
              </div>
              <div className="flex items-center justify-between">
                <div className="flex gap-1">
                {dummy.map((_ ,indx)=>{return <FaStar key={indx}/>})}
                </div>
                <span>1 Week Ago</span>
              </div>
              <p className="text-gray-700 mt-2 line-clamp-3 w-[50%]">{review.comment}</p>
              
              <button  className="bg-slate-50 p-1 my-4 flex items-center font-semibold underline">Show more{<FaAngleRight />}</button>
            </div>
          ))}
        </div>
      ) : (
